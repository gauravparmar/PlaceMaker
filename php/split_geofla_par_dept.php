...
$str_query = "SELECT code_dept,nom_dept,round(ST_XMin(g)) xmin,round(ST_XMax(g)) xmax,round(ST_YMin(g)) ymin,round(ST_YMax(g)) ymax
							...
							(
								...
								UNION ALL
								SELECT 'RdM','Reste du Monde',ST_Transform(ST_SetSRID(ST_MakeBox2D(ST_Point(-170,-80),ST_Point(170,80)),4326),900913)
							)a
							ORDER BY substr(code_dept,1,1),substr(code_dept,2,1),substr(code_dept,3,1);";
